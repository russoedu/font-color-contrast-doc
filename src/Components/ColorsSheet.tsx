...
const colors = generator(6, '0369BDF')
...
export function ColorsSheet () {
  const [splicer, setSplicer] = useState(0)
  const spliceLimit = 290
  let i = 0

  function handleSlider(event: any) {
    setSplicer(event.target.value);
  }

  const listColours = testColors.slice(splicer * spliceLimit, splicer * spliceLimit + spliceLimit).map((color) => {
      const divStyle = {
  ...
  return (
    <>
      <div className="slidecontainer">
        ...
        />
        <p>Page {Number(splicer) + 1} of {(testColors.length / spliceLimit).toFixed(0)} (from {testColors[splicer * spliceLimit]} to {testColors[splicer * spliceLimit + spliceLimit - 1] || '#FFFFFF'}</p>
      </div>
